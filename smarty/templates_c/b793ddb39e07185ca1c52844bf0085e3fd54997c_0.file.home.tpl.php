<?php
/* Smarty version 3.1.31, created on 2025-04-11 13:36:53
  from "D:\Alexis Code Projects\The5AM-Website\1-Full-Site\public_html\smarty\templates\home.tpl" */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.31',
  'unifunc' => 'content_67f91af54ccf06_42800674',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    'b793ddb39e07185ca1c52844bf0085e3fd54997c' => 
    array (
      0 => 'D:\\Alexis Code Projects\\The5AM-Website\\1-Full-Site\\public_html\\smarty\\templates\\home.tpl',
      1 => 1744378459,
      2 => 'file',
    ),
  ),
  'includes' => 
  array (
  ),
),false)) {
function content_67f91af54ccf06_42800674 (Smarty_Internal_Template $_smarty_tpl) {
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Public+Sans:wght@300;400&display=swap" rel="stylesheet">
    <!-- Font Awesome CDN -->
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css" rel="stylesheet">
    <!-- Bootstrap CSS (CDN) -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="/css/bootstrap.min.css">
    <link rel="stylesheet" href="/css/style.css">

    <title>The 5 AM</title>

    <?php echo '<script'; ?>
 src="/js/jquery-3.6.0.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="/js/bootstrap.min.js"><?php echo '</script'; ?>
>
    <?php
$__section_teller_0_saved = isset($_smarty_tpl->tpl_vars['__smarty_section_teller']) ? $_smarty_tpl->tpl_vars['__smarty_section_teller'] : false;
$__section_teller_0_loop = (is_array(@$_loop=$_smarty_tpl->tpl_vars['jsInclude']->value) ? count($_loop) : max(0, (int) $_loop));
$__section_teller_0_total = $__section_teller_0_loop;
$_smarty_tpl->tpl_vars['__smarty_section_teller'] = new Smarty_Variable(array());
if ($__section_teller_0_total != 0) {
for ($__section_teller_0_iteration = 1, $_smarty_tpl->tpl_vars['__smarty_section_teller']->value['index'] = 0; $__section_teller_0_iteration <= $__section_teller_0_total; $__section_teller_0_iteration++, $_smarty_tpl->tpl_vars['__smarty_section_teller']->value['index']++){
?>
        <?php echo '<script'; ?>
 src="<?php echo $_smarty_tpl->tpl_vars['jsInclude']->value[(isset($_smarty_tpl->tpl_vars['__smarty_section_teller']->value['index']) ? $_smarty_tpl->tpl_vars['__smarty_section_teller']->value['index'] : null)];?>
"><?php echo '</script'; ?>
>
    <?php
}
}
if ($__section_teller_0_saved) {
$_smarty_tpl->tpl_vars['__smarty_section_teller'] = $__section_teller_0_saved;
}
?>
</head>


<body>
    <div class="background"></div>

    <?php echo $_smarty_tpl->tpl_vars['nav']->value;?>


    <div class='row'>
        <?php echo $_smarty_tpl->tpl_vars['inhoud']->value;?>

    </div>

    <?php echo '<script'; ?>
 src="/js/popper.min.js"><?php echo '</script'; ?>
>
    <!-- Bootstrap JS (CDN) -->
    <?php echo '<script'; ?>
 src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript para mover el navbar -->
    <?php echo '<script'; ?>
 src="/js/scroll.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript read more / read less -->
    <?php echo '<script'; ?>
 src="/js/more_about.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript photos -->
    <?php echo '<script'; ?>
 src="/js/photos.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript navbar-mobile -->
    <?php echo '<script'; ?>
 src="/js/collapse_nav_bar.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript navbar-active -->
    <?php echo '<script'; ?>
 src="/js/navbar_active.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript more_videos -->
    <?php echo '<script'; ?>
 src="/js/more_videos.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript more_events -->
    <?php echo '<script'; ?>
 src="/js/more_events.js"><?php echo '</script'; ?>
>
    <!-- Script de JavaScript more_press -->
    <?php echo '<script'; ?>
 src="/js/more_press.js"><?php echo '</script'; ?>
>

</body>

</html><?php }
}
